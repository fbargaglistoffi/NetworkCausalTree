...
$$ \scriptsize \tau_{(w,g;w',g')}(l(\mathbf{x})) = \frac{1}{N(l(\mathbf{x}))}\sum_{k:1}^{K}\sum_{i:1}^{n_k}Y_{ik}(w,g)I(\mathbf{X_{ik}} \in l(\mathbf{x}))  - \frac{1}{N(l(\mathbf{x}))}\sum_{k:1}^{K}\sum_{i:1}^{n_k}Y_{ik}(w',g')I(\mathbf{X_{ik}} \in l(\mathbf{x}))\\ $$ where $N(l(\mathbf{x}))$ is the number of units within the leaf $l(\mathbf{x})$.
