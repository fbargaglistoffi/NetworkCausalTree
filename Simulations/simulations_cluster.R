...
system.time({
  matrix <- foreach(j = 1:nsim,  .combine = 'comb', .multicombine = TRUE) %dopar% {
    ...
    for (i in seq)   {
      ...
      if (correct==2){ 
        # tau1000_1 & tau1101_1 & tau1000_1 & tau_1101_2
    ...
    ## Return the values
    list(correct.rules, tau.est.1000, se.tau.est.1000, vi.x1, vi.x2, vi.X)
...
vi_x1 <- na.omit(matrix[[4]])
vi_x2 <- na.omit(matrix[[5]])
vi_X <- na.omit(matrix[[6]])
...
for(i in seq){
  avg_tau_1000_1[which(seq==i)] <- mean( tau_est_1000[,which(seq==i)][tau_est_1000[,which(seq==i)]>=0] , na.rm = TRUE )
  avg_tau_1000_2[which(seq==i)] <- mean( tau_est_1000[,which(seq==i)][tau_est_1000[,which(seq==i)]<=0] , na.rm = TRUE )
}

avg_se_1000_1 <-  avg_se_1000_2 <- c()
for(i in seq){
  avg_se_1000_1[which(seq==i)] <- mean( se_tau_est_1000[,which(seq==i)][tau_est_1000[,which(seq==i)]>=0] , na.rm = TRUE )
  avg_se_1000_2[which(seq==i)] <- mean( se_tau_est_1000[,which(seq==i)][tau_est_1000[,which(seq==i)]<=0] , na.rm = TRUE )
}


## Mean Squared Error & Bias
mse_tau_est_1000 <- bias_tau_est_1000 <- c()
...
results_nctree <- cbind(avg_correct_rules,
                        ...
                        avg_tau_1000_1, avg_tau_1000_2,
                        avg_se_1000_1, avg_se_1000_2,
                        coverage_est_1000, 
                        vi_x1, vi_x2, vi_X)
...
## Save the Results
write.csv(results_nctree, file = "one_main_effect_1500.csv")


##################################
##   2 RULES &  1 SPIL. EFFECT  ##
##################################
...
system.time({
  matrix <- foreach(j = 1:nsim,  .combine = 'comb', .multicombine = TRUE) %dopar% {
    ...
    for (i in seq)   {
      ...
      ## Generate Outcome
      y <- y00*(1-w)*(1-g) + y01*(1-w)*g
      
      
      ## Run the  whole function
      SNCT <- NetworkCausalTrees(effweights = c(0,0,0,1), method = "singular", # composite
                                 output = "estimation", # detection, estimation
                                 A = adiac_matrix,
                                 p = rep(probT,n), Ne = NeighNum,
                                 W = w, Y = y, X = X, M = M, G = g,
      ...
      ## Extract Values for the Estimation Set (Get 0 if the Causal Rule was not identified)
      if (correct==2){ 
        # eta0100_1 & eta1101_1 & eta0100_1 & eta_1101_2
    ...
    ## Return the values
    list(correct.rules, eta.est.0100, se.eta.est.0100, vi.x1, vi.x2, vi.X)
...
## Extract the Returned Values
correct_rules <- na.omit(matrix[[1]])
eta_est_0100 <- na.omit(matrix[[2]])
se_eta_est_0100 <- na.omit(matrix[[3]])
vi_x1 <- na.omit(matrix[[4]])
vi_x2 <- na.omit(matrix[[5]])
vi_X <- na.omit(matrix[[6]])
...
for(i in seq){
  avg_se_0100_1[which(seq==i)] <- mean( se_eta_est_0100[,which(seq==i)][eta_est_0100[,which(seq==i)]>=0] , na.rm = TRUE )
  avg_se_0100_2[which(seq==i)] <- mean( se_eta_est_0100[,which(seq==i)][eta_est_0100[,which(seq==i)]<=0] , na.rm = TRUE )
}


## Mean Squared Error & Bias
mse_eta_est_0100 <- bias_eta_est_0100 <- c()
...
results_nctree <- cbind(avg_correct_rules,
                        ...
                        avg_eta_0100_1, avg_eta_0100_2,
                        avg_se_0100_1, avg_se_0100_2,
                        coverage_est_0100, 
                        vi_x1, vi_x2, vi_X)
colnames(results_nctree) <- c("correct_rules",
                              "MSE eta(01,00)", 
...
## Save the Results
write.csv(results_nctree, file = "one_spillover_effect_1500.csv")
  ...
  matrix <- foreach(j = 1:nsim,  .combine = 'comb', .multicombine = TRUE) %dopar% {
    ...
    for (i in seq)   {
      ...
      if (correct==2){ 
        ...
        effects.est1000_1 <- SNCT$EFF1000_EST[which(SNCT$RULE.NUM==1)]
        effects.est1000_2 <- SNCT$EFF1000_EST[which(SNCT$RULE.NUM==2)] 
        effects.est1101_1 <- SNCT$EFF1101_EST[which(SNCT$RULE.NUM==1)] 
        effects.est1101_2 <- SNCT$EFF1101_EST[which(SNCT$RULE.NUM==2)] 
        se.est1000_1 <- SNCT$SE1000_EST[which(SNCT$RULE.NUM==1)]
        se.est1000_2 <- SNCT$SE1000_EST[which(SNCT$RULE.NUM==2)] 
        se.est1101_1 <- SNCT$SE1101_EST[which(SNCT$RULE.NUM==1)] 
        se.est1101_2 <- SNCT$SE1101_EST[which(SNCT$RULE.NUM==2)] 
      }  
    ...
    ## Return the values
    list(correct.rules, tau.est.1000, tau.est.1101, se.tau.est.1000, se.tau.est.1101, vi.x1, vi.x2, vi.X)
...
se_tau_est_1000[se_tau_est_1000==0] <- NA 
se_tau_est_1101[se_tau_est_1101==0] <- NA 
...
avg_tau_1101_1 <-  avg_tau_1101_2 <- c()
for(i in seq){
  avg_tau_1101_1[which(seq==i)] <- mean( tau_est_1101[,which(seq==i)][tau_est_1101[,which(seq==i)]>=0] , na.rm = TRUE )
  avg_tau_1101_2[which(seq==i)] <- mean( tau_est_1101[,which(seq==i)][tau_est_1101[,which(seq==i)]<=0] , na.rm = TRUE )
}

avg_se_1101_1 <-  avg_se_1101_2 <- c()
for(i in seq){
  avg_se_1101_1[which(seq==i)] <- mean( se_tau_est_1101[,which(seq==i)][tau_est_1101[,which(seq==i)]>=0] , na.rm = TRUE )
  avg_se_1101_2[which(seq==i)] <- mean( se_tau_est_1101[,which(seq==i)][tau_est_1101[,which(seq==i)]<=0] , na.rm = TRUE )
}
...
results_nctree <- cbind(avg_correct_rules,
                        ...
                        coverage_est_1000,
                        mse_tau_est_1101, bias_tau_est_1101,
                        avg_tau_1101_1, avg_se_1101_1,
                        avg_tau_1101_2, avg_se_1101_2,
                        coverage_est_1101,
                        vi_x1, vi_x2, vi_X)
...
colnames(results_nctree) <- c("correct_rules",
                              ...
                              "Coverage tau(11,01)",
                              "VI x1", "VI x2", "VI X")
write.csv(results_nctree, file = "two_main_effects_1500.csv")
...
system.time({
  matrix <- foreach(j = 1:nsim,  .combine = 'comb', .multicombine = TRUE) %dopar% {
    ...
    ## Initialize Matrices
    correct.rules <- data.frame()
    eta.est.1110 <- data.frame()
    eta.est.0100 <- data.frame()
    se.eta.est.1110 <- data.frame()
    se.eta.est.0100 <- data.frame()
    vi.x1 <- vi.x2 <- vi.X <- data.frame()
    ...
    for (i in seq)   {
      ...
      if (correct==2){ 
        # eta1110_1 & eta0100_1 & eta1110_1 & eta_0100_2
        effects.est1110_1 <- SNCT$EFF1110_EST[which(SNCT$RULE.NUM==1)]
        effects.est1110_2 <- SNCT$EFF1110_EST[which(SNCT$RULE.NUM==2)] 
        effects.est0100_1 <- SNCT$EFF0100_EST[which(SNCT$RULE.NUM==1)] 
        effects.est0100_2 <- SNCT$EFF0100_EST[which(SNCT$RULE.NUM==2)] 
        se.est1110_1 <- SNCT$SE1110_EST[which(SNCT$RULE.NUM==1)]
        se.est1110_2 <- SNCT$SE1110_EST[which(SNCT$RULE.NUM==2)] 
        se.est0100_1 <- SNCT$SE0100_EST[which(SNCT$RULE.NUM==1)] 
        se.est0100_2 <- SNCT$SE0100_EST[which(SNCT$RULE.NUM==2)] 
      }  
    ...
    ## Return the values
    list(correct.rules, eta.est.1110, eta.est.0100, se.eta.est.1110, se.eta.est.0100, vi.x1, vi.x2, vi.X)
...
eta_est_0100[eta_est_0100==0] <- NA
se_eta_est_1110[se_eta_est_1110==0] <- NA 
se_eta_est_0100[se_eta_est_0100==0] <- NA 

## Average Effects and Average SD
avg_eta_1110_1 <-  avg_eta_1110_2 <- c()
for(i in seq){
  avg_eta_1110_1[which(seq==i)] <- mean( eta_est_1110[,which(seq==i)][eta_est_1110[,which(seq==i)]>=0] , na.rm = TRUE )
  avg_eta_1110_2[which(seq==i)] <- mean( eta_est_1110[,which(seq==i)][eta_est_1110[,which(seq==i)]<=0] , na.rm = TRUE )
}

avg_se_1110_1 <-  avg_se_1110_2 <- c()
for(i in seq){
  avg_se_1110_1[which(seq==i)] <- mean( se_eta_est_1110[,which(seq==i)][eta_est_1110[,which(seq==i)]>=0] , na.rm = TRUE )
  avg_se_1110_2[which(seq==i)] <- mean( se_eta_est_1110[,which(seq==i)][eta_est_1110[,which(seq==i)]<=0] , na.rm = TRUE )
}
...
## Create a Matrix for the Results
results_nctree <- cbind(avg_correct_rules,
                        mse_eta_est_1110, bias_eta_est_1110,
                        avg_eta_1110_1, avg_se_1110_1,
                        avg_eta_1110_2, avg_se_1110_2,
                        coverage_est_1110,
                        mse_eta_est_0100, bias_eta_est_0100,
...
colnames(results_nctree) <- c("correct_rules",
                              ...
                              "Coverage eta(01,00)",
                              "VI x1", "VI x2", "VI X")
write.csv(results_nctree, file = "two_spillover_effects_1500.csv")
...
system.time({
  matrix <- foreach(j = 1:nsim,  .combine = 'comb', .multicombine = TRUE) %dopar% {
    ...
    for (i in seq)   {
      ...
      # Tau1000
      tau1000 <- rep(0, n)
      tau1000[x1==0 & x2==0] <- i # Rule Number 1
      tau1000[x1==0 & x2==1] <- -i # Rule Number 2, different HDVs
      
      # Tau 1101
      tau1101 <- rep(0, n)
      tau1101[x1==0 & x2==0] <- i # Rule Number 1
      tau1101[x1==1 & x3==1] <- -i # Rule Number 3, different HDVs
      ...
      if (correct==3) {
        ...
        effects.est1000_2 <- SNCT$EFF1000_EST[which(SNCT$RULE.NUM==2)]
        effects.est1101_1 <- SNCT$EFF1101_EST[which(SNCT$RULE.NUM==1)] 
        effects.est1101_2 <- SNCT$EFF1101_EST[which(SNCT$RULE.NUM==3)]
        se.est1000_1 <- SNCT$SE1000_EST[which(SNCT$RULE.NUM==1)] 
        se.est1000_2 <- SNCT$SE1000_EST[which(SNCT$RULE.NUM==2)]
        se.est1101_1 <- SNCT$SE1101_EST[which(SNCT$RULE.NUM==1)] 
        se.est1101_2 <- SNCT$SE1101_EST[which(SNCT$RULE.NUM==3)]
      }
      
      if (correct==2){ 
        # tau1000_1 & tau1101_1 & tau1000_2
        ...
        # tau1000_1 & tau1101_1 & tau1101_2
        if (length((which(SNCT$RULE.NUM==1 | SNCT$RULE.NUM==3)))==2){
          effects.est1000_1 <- SNCT$EFF1000_EST[which(SNCT$RULE.NUM==1)]
          effects.est1000_2 <- 0
          effects.est1101_1 <- SNCT$EFF1101_EST[which(SNCT$RULE.NUM==1)] 
          effects.est1101_2 <- SNCT$EFF1000_EST[which(SNCT$RULE.NUM==3)] 
          se.est1000_1 <- SNCT$SE1000_EST[which(SNCT$RULE.NUM==1)]
          se.est1000_2 <- 0
          se.est1101_1 <- SNCT$SE1101_EST[which(SNCT$RULE.NUM==1)] 
          se.est1101_2 <- SNCT$SE1101_EST[which(SNCT$RULE.NUM==3)]
        ...
        if (length((which(SNCT$RULE.NUM==2 | SNCT$RULE.NUM==3)))==2){
          ...
          effects.est1101_2 <- SNCT$EFF1101_EST[which(SNCT$RULE.NUM==3)]
          se.est1000_1 <- 0
          se.est1000_2 <- SNCT$EFF1000_EST[which(SNCT$RULE.NUM==2)]
          se.est1101_1 <- 0
          se.est1101_2 <- SNCT$SE1101_EST[which(SNCT$RULE.NUM==3)]
        }
    ...
    list(correct.rules, tau.est.1000, tau.est.1101, se.tau.est.1000, se.tau.est.1101, vi.x1, vi.x2, vi.x3, vi.X)
  }
})
...
write.csv(results_nctree, file = "two_main_effects_three_rules_1500.csv")
...
system.time({
  matrix <- foreach(j = 1:nsim,  .combine = 'comb', .multicombine = TRUE) %dopar% {
    ...
    for (i in seq)   {
      ...
      # Tau 1101
      tau1101 <- rep(0, n)
      tau1101[x1==0 & x2==0] <- i*1.50 # Rule Number 1 (EFFECT SIZE)
      tau1101[x1==1 & x3==1] <- -i*1.50 # Rule Number 3, different HDVs (EFFECT SIZE)
...
colnames(results_nctree) <- c("correct_rules",
                              ...
                              "Coverage tau(11,01)",
                              "VI x1", "VI x2", "VI x3", "VI X")
write.csv(results_nctree, file = "two_main_effects_three_rules_effect_size_1500.csv")
