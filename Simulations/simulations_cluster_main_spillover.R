...
system.time({
  matrix <- foreach(j = 1:nsim,  .combine = 'comb', .multicombine = TRUE) %dopar% {
    ...
    se.tau.est.1000.spil <- data.frame()
    se.eta.est.0100.spil <- data.frame()
    vi.x1.spil <- vi.x2.spil <- vi.x3.spil <- vi.X.spil <- data.frame()
    ...
    for (i in seq)   {
      ...
      if (correct==2){ 
        ...
        if (length((which(SNCT$RULE.NUM==1 | SNCT$RULE.NUM==3)))==2){
          ...
          effects.est1000_2 <- 0
          effects.est0100_1 <- SNCT$EFF0100_EST[which(SNCT$RULE.NUM==1)] 
          effects.est0100_2 <- SNCT$EFF1000_EST[which(SNCT$RULE.NUM==3)] 
          se.est1000_1 <- SNCT$SE1000_EST[which(SNCT$RULE.NUM==1)]
          se.est1000_2 <- 0
          se.est0100_1 <- SNCT$SE0100_EST[which(SNCT$RULE.NUM==1)] 
          se.est0100_2 <- SNCT$SE0100_EST[which(SNCT$RULE.NUM==3)]
      ...
      SNCT_main$RULE.NUM[which(rule.main=="data_tree$X.1<1 & data_tree$X.2<1" | rule.main=="data_tree$X.2<1 & data_tree$X.1<1")] <- 1
      SNCT_main$RULE.NUM[which(rule.main=="data_tree$X.1<1 & data_tree$X.2>=1" | rule.main=="data_tree$X.2>=1 & data_tree$X.1<1")] <- 2
      SNCT_main$RULE.NUM[which(rule.main=="data_tree$X.1>=1 & data_tree$X.3>=1" | rule.main=="data_tree$X.3>=1 & data_tree$X.1>=1")] <-3
      ...
      vi.x3.main[j, which(seq==i)] <- str_detect(SNCT_main$FILTER, "X.3")[2]
      vi.X.main[j, which(seq==i)] <-  cbind(str_detect(SNCT_main$FILTER, "X.4")[2] |
                                              str_detect(SNCT_main$FILTER, "X.5")[2] |  
                                              str_detect(SNCT_main$FILTER, "X.6")[2] |
                                              str_detect(SNCT_main$FILTER, "X.7")[2] |  
                                              str_detect(SNCT_main$FILTER, "X.8")[2] |
      ...
      if (correct==1){ 
        ...
        # tau0100_2
        if (length((which(SNCT_main$RULE.NUM==3)))==1){
          effects.est1000_1 <- 0
          effects.est1000_2 <- 0
          effects.est0100_1 <- 0
          effects.est0100_2 <- SNCT_main$EFF0100_EST[which(SNCT_main$RULE.NUM==3)]
      ...
      SNCT_spil$RULE.NUM[which(rule.spil=="data_tree$X.1<1 & data_tree$X.2<1" | rule.spil=="data_tree$X.2<1 & data_tree$X.1<1")] <- 1
      SNCT_spil$RULE.NUM[which(rule.spil=="data_tree$X.1<1 & data_tree$X.2>=1" | rule.spil=="data_tree$X.2>=1 & data_tree$X.1<1")] <- 2
      SNCT_spil$RULE.NUM[which(rule.spil=="data_tree$X.1>=1 & data_tree$X.3>=1" | rule.spil=="data_tree$X.3>=1 & data_tree$X.1>=1")] <-3
      ...
      vi.x3.spil[j, which(seq==i)] <- str_detect(SNCT_spil$FILTER, "X.3")[2]
      vi.X.spil[j, which(seq==i)] <-  cbind(str_detect(SNCT_spil$FILTER, "X.4")[2] |
                                              str_detect(SNCT_spil$FILTER, "X.5")[2] |  
                                              str_detect(SNCT_spil$FILTER, "X.6")[2] |
                                              str_detect(SNCT_spil$FILTER, "X.7")[2] |  
                                              str_detect(SNCT_spil$FILTER, "X.8")[2] |
      ...
      if (correct==1){ 
        ...
        # tau0100_2
        if (length((which(SNCT_spil$RULE.NUM==3)))==1){
          effects.est1000_1 <- 0
          effects.est1000_2 <- 0
          effects.est0100_1 <- 0
          effects.est0100_2 <- SNCT_spil$EFF0100_EST[which(SNCT_spil$RULE.NUM==3)]
...
colnames(results_nctree) <- c("correct_rules",
                              ...
                              "Coverage eta(01,00)",
                              "VI x1", "VI x2", "VI x3", "VI X")
write.csv(results_nctree, file = "two_main_spillover_three_effects_singular_main_1500.csv")
